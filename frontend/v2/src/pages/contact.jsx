...
export default function Contact() {
  ...
  return (
    <div className="bg-card min-h-screen">
      ...
      {/* Hero */}
      <section className="pt-32 pb-16 px-6 bg-surface border-b border-border">
        <div className="max-w-3xl mx-auto text-center">
          <span className="inline-block bg-primary/10 text-primary text-sm font-semibold px-4 py-2 rounded-full mb-5">💬 Get In Touch</span>
          <h1 className="text-4xl md:text-5xl font-semibold text-foreground mb-3">We're Here to Help</h1>
      ...
      <section className="section-pad bg-card">
        <div className="max-w-5xl mx-auto grid lg:grid-cols-5 gap-12">
          ...
          <div className="lg:col-span-2 space-y-6">
            {[
              { icon: <FiMail size={20} />, label: "Email", value: "support@nammaconnect.in", color: "bg-primary/10 text-primary" },
              { icon: <FiPhone size={20} />, label: "Phone", value: "+91 98765 43210", color: "bg-primary/10 text-primary" },
              { icon: <FiMapPin size={20} />, label: "Address", value: "Bengaluru, Karnataka, India", color: "bg-primary/10 text-primary" },
            ].map(c => (
              <motion.div key={c.label} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}
                className="flex items-start gap-4 p-5 bg-surface rounded-2xl border border-border"
              >
                <div className={`w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 ${c.color}`}>{c.icon}</div>
                <div>
                  <p className="text-xs text-muted-foreground font-semibold uppercase tracking-widest">{c.label}</p>
            ...
            {/* Socials */}
            <div className="pt-4">
              <p className="text-xs text-muted-foreground font-semibold uppercase tracking-widest mb-4">Follow us</p>
              <div className="flex gap-3">
                {[FiInstagram, FiTwitter, FiFacebook, FiYoutube].map((Icon, i) => (
                  <a key={i} href="#" className="w-10 h-10 rounded-xl border border-border bg-card flex items-center justify-center text-muted-foreground hover:text-primary hover:border-primary/30 hover:shadow-md transition-all">
                    <Icon size={17} />
                  </a>
                ))}
              </div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            className="lg:col-span-3 bg-surface rounded-2xl p-8 border border-border shadow-sm"
          >
            {sent && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                className="mb-6 p-4 bg-primary/10 border border-primary/30 rounded-xl text-primary text-sm font-medium"
              >
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="text-xs font-semibold text-muted-foreground block mb-1.5">Topic</label>
