...
const Field = ({ label, type = "text", placeholder, icon, required, value, onChange, inputMode, maxLength, as }) => (
  <div>
    ...
    <div className="relative">
      {icon && <span className="absolute left-3 top-1/2 -translate-y-1/2 text-purple-500">{icon}</span>}
      {as === "select" ? (
        ...
        <input
          ...
          className={`input-field text-sm ${icon ? "pl-10" : ""} focus:border-purple-400`}
...
export default function CreatorRegister() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      localStorage.setItem("ng_user", JSON.stringify({ ...user, role: "creator", name: form.name }));
      setSuccess(true);
  ...
  if (user?.role === "creator") {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        ...
        >
          <FiCheckCircle size={56} className="text-purple-500 mx-auto mb-5" />
          <h2 className="text-2xl font-black mb-2">Already Registered</h2>
          <p className="text-slate-500 text-sm mb-6">You're already onboarded as a creator.</p>
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-600 to-indigo-700 flex items-center justify-center">
        <motion.div
          ...
          className="text-center bg-white rounded-3xl p-14 shadow-2xl max-w-sm"
        >
          <div className="w-20 h-20 bg-purple-100 rounded-full flex items-center justify-center mx-auto mb-6">
            <FiCheckCircle size={40} className="text-purple-600" />
          </div>
          <h2 className="text-2xl font-black mb-2">You're a Creator! 🎬</h2>
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <div className="flex flex-col lg:flex-row flex-1 pt-16">
        ...
        <motion.aside
          ...
          className="lg:w-[38%] bg-gradient-to-b from-purple-600 via-violet-700 to-indigo-800 text-white p-12 lg:p-16 flex flex-col justify-between relative overflow-hidden"
        >
          <div className="absolute inset-0 overflow-hidden pointer-events-none">
            <div className="absolute -top-32 -right-32 w-80 h-80 bg-white/10 rounded-full blur-3xl" />
            <div className="absolute -bottom-32 -left-32 w-80 h-80 bg-indigo-900/30 rounded-full blur-3xl" />
          </div>
          ...
          <div className="relative z-10">
            ...
            <h1 className="text-4xl lg:text-5xl font-black leading-tight mb-4">Become a<br />Creator 🎬</h1>
            <p className="text-purple-200 text-base leading-relaxed mb-10">
              Partner with farms, showcase your content, and build your agri-creator brand on NammaGig.
            </p>
          ...
          <div className="relative z-10">
            {(user?.name || user?.full_name) && (
              <div className="bg-white/15 backdrop-blur-sm border border-white/20 rounded-2xl p-4 flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs text-purple-300 font-bold uppercase tracking-widest">Logged in as</p>
                  <p className="font-black">{user.name || user.full_name}</p>
                </div>
              </div>
            )}
          </div>
        ...
        >
          <h2 className="text-2xl font-black text-slate-900 mb-1">Creator Profile</h2>
          <p className="text-slate-400 text-xs uppercase tracking-widest font-bold mb-8">Tell us about you and your content</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {/* Content niche */}
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.22 }} className="pt-2 border-t border-slate-100">
              <p className="text-xs font-black text-slate-400 uppercase tracking-widest mb-4">Content & Pricing</p>
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
              <label className="text-xs font-bold text-slate-500 block mb-1.5 uppercase tracking-widest">Bio</label>
              <textarea rows={3} value={form.bio} onChange={upd("bio")} placeholder="Tell farms what makes you special..."
                className="input-field text-sm resize-none w-full focus:border-purple-400"
              />
            </motion.div>

            {/* Work experience */}
            <motion.label initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.33 }}
              className="flex items-center gap-3 cursor-pointer bg-purple-50 border border-purple-100 px-4 py-3 rounded-2xl"
            >
              <input
                type="checkbox" checked={form.has_work_experience} onChange={upd("has_work_experience")}
                className="w-4 h-4 accent-purple-600"
              />
              <span className="text-sm font-semibold text-slate-700">I have prior work experience in content creation</span>
            </motion.label>

            {/* ID */}
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.36 }} className="pt-2 border-t border-slate-100">
            ...
            <motion.button
              ...
              className="w-full bg-gradient-to-r from-purple-600 to-indigo-700 text-white font-black py-4 rounded-2xl text-base shadow-xl shadow-purple-200 transition-all disabled:opacity-60 flex items-center justify-center gap-3 mt-2"
            >
              {loading
